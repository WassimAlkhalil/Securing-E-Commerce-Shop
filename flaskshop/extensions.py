...
def get_locale():
    """Secure locale selector.

    - Accepts `lang` from query string.
    - Whitelists against configured languages.
    - Normalizes to Babel locale codes to match available translations.
    """

    # Allowed short language codes from config, e.g., {"en": "English", "bg": "Български"}
    languages = (current_app.config.get("LANGUAGES") or {}).keys()

    requested = request.args.get("lang")
    if requested:
        # Normalize and validate: only accept known short codes
        requested = requested.strip().lower()
        if requested in languages:
            session["lang"] = requested
        # else: ignore invalid values silently (security: no arbitrary session data)

    # Default to English short code
    short = session.get("lang", "en")

    # Map short codes to Babel locale codes that exist in /translations
    # Extend as needed if more languages are added.
    mapping = {
        "en": current_app.config.get("BABEL_DEFAULT_LOCALE", "en_US"),
        "bg": "bg_BG",
    }

    return mapping.get(short, current_app.config.get("BABEL_DEFAULT_LOCALE", "en_US"))
