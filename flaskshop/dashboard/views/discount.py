...
from flaskshop.dashboard.utils import item_del, wrap_partial
...
def vouchers_manage(id=None):
    if id:
        voucher_id = int(id) if id else None
        voucher = Voucher.get_by_id(voucher_id)
        form = VoucherForm(obj=voucher, voucher_id=voucher_id)
    else:
        voucher = Voucher()
        form = VoucherForm()
