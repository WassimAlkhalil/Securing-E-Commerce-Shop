...
def update_cartline(id):
    ...
    if quantity <= 0:
        ...
        result = line.variant.check_enough_stock(quantity)
        if not result:
            quantity = min(quantity, line.variant.stock)
    ...
    if cart:
        ...
        response["cart"]["numItems"] = 0
        response["cart"]["numLines"] = 0
        response["subtotal"] = "$0"
        response["total"] = "$0"
...
def checkout_shipping():
    ...
    user_address = None
    form = AddressForm(request.form, meta={'csrf': False})
