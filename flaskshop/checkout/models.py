...
class Cart(Model):
    ...
    def total(self):
        ...
        return max(0, self.subtotal + self.shipping_method_price - self.discount_amount)
    ...
    def lines(self):
        ...
        # Filter cart lines to only include items with quantity > 0
        # This prevents showing deleted items (quantity=0) in cart display and checkout
        # Example: If user removes an item, quantity becomes 0, this line is excluded from cart.lines
        return CartLine.query.filter(CartLine.cart_id == self.id, CartLine.quantity > 0).all()
