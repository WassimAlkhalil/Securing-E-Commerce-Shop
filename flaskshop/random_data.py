...
class SaleorProvider(BaseProvider):
    ...
    def shipping_method(self):
        #fixed by swarup
        return random.choice(ShippingMethod.query.all()) # nosec B311 – non-security test data
...
DASHBOARD_MENUS = [
    ...
    # Add courier shipping simulation
    # @author: Nebil Müren - cas3322
    {"title": "SHIPPING", "endpoint": "shipping_list", "icon_cls": "fa-truck"},
...
def create_products_by_type(
    ...
    for dummy in range(how_many):
        ...
        if create_images:
            ...
            #fixed by swarup
            create_product_images(product, random.randrange(1, 5), type_placeholders) # nosec B311 – used for non-security test data
        ...
        for i, variant_price in enumerate(variants_with_prices, start=1337):
            ...
            sku = f"{product.id}-{i}"
            create_variant(product, sku=sku, title=title, price_override=price)
...
def create_product(**kwargs):
    ...
    defaults = {
        ...
        "description": "\n\n".join(description),
        #fixed by swarup
        "is_featured": random.choice([0, 1]),# nosec B311 – used for non-security test data
...
def set_product_attributes(product, product_type):
    ...
    for product_attribute in product_type.product_attributes:
        #fixed by swarup
        value = random.choice(product_attribute.values) # nosec B311 – non-security test data
...
def create_product_images(product, how_many, placeholder_dir):
    ...
    for dummy in range(how_many):
        #fixed by swarup
        image_name = random.choice(list(placeholder_root.iterdir())) # nosec B311 – non-security test data
...
def create_admin():
    # fixed by swarup
    user = User.create(
        username="admin", email="admin@163.com",password="admin", is_active=True #nosec
    )
    ...
    # fixed by swarup
    user = User.create(username="op", email="op@163.com",password="op", is_active=True)#nosec
    UserRole.create(user_id=user.id, role_id=3)
    yield f"Admin {user.username} created"
    #fixed by swarup
    user = User.create(
        username="editor", email="editor@163.com", password="editor", is_active=True#nosec
    )
...
def create_fake_order(discounts):
    ...
    status = random.choice(list(OrderStatusKinds)).value # nosec B311 — non-cryptographic demo data
    order_data = {
        "user_id": user.id,
    ...
    lines = create_order_lines(order, discounts, random.randrange(1, 5))# nosec B311 — test data generation
...
def create_order_line(order, discounts):
    ...
    quantity = random.randrange(1, 5)# nosec B311 — used for demo/test purposes
...
def create_payment(order):
    #fixed by swarup
    status = random.choice(list(PaymentStatusKinds)).value# nosec B311 — used for demo/test purposes
...
def create_fake_sale():
    sale = Sale.create(
        ...
        discount_value=random.choice([10, 20, 30, 40, 50]),# nosec B311 — used for demo/test purposes
    )
...
def create_vouchers():
    ...
    # Added default value voucher
    # @author: Nebil Müren - cas3322
    defaults = {
        "type_": VoucherTypeKinds.value.value,
        "title": "Generous discount",
        "discount_value_type": DiscountValueTypeKinds.fixed.value,
        "discount_value": 250,
    }

    voucher, created = Voucher.get_or_create(code="GENEROUS", **defaults)
    if created:
        yield f"Voucher #{voucher.id}"
    else:
        yield "Value voucher already exists"
