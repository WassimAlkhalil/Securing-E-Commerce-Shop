...
class Category(Model):
    ...
    def get_product_by_category(cls, category_id, page):
        ...
        # Filter by actual displayed price (includes discounts)
        # This must be done after fetching because price is a computed property
        price_from = ctx.get('price_from')
        price_to = ctx.get('price_to')
        filtered_items = []
        ...
        # Sort by actual displayed price when sorting by price
        # (SQL sorts by basic_price, but we need to sort by computed price)
        if ctx.get('now_sorted_by') == 'basic_price':
            filtered_items.sort(key=lambda p: float(p.price), reverse=ctx.get('is_descending', False))
        
        ctx.update(object=category, pagination=pagination, products=filtered_items)
        return ctx
...
class ProductCollection(Model):
    ...
    def get_product_by_collection(cls, collection_id, page):
        ...
        ctx.update(object=collection, pagination=pagination, products=filtered_items)
        return ctx
...
class ProductFeedback(Model):
    ...
    nickname = Column(db.String(64), nullable=False)
    packaging_rating = Column(db.SmallInteger(), nullable=False, default=0)
    delivery_rating = Column(db.SmallInteger(), nullable=False, default=0)
    item_rating = Column(db.SmallInteger(), nullable=False, default=0)
    comment = Column(db.Text())
    __table_args__ = (
        UniqueConstraint("product_id", "user_id", name="uix_product_feedback_product_user"),
    )

    @property
    def overall_rating(self):
        total = self.packaging_rating + self.delivery_rating + self.item_rating
        return round(total / 3.0, 2)

    @classmethod
    def _refresh_product_stats(cls, product_id):
        product = Product.get_by_id(product_id)
        if not product:
            return
        feedbacks = cls.query.filter_by(product_id=product_id).all()
        if not feedbacks:
            product.update(rating=5.0, review_count=0)
            return
        average = round(
            sum(feedback.overall_rating for feedback in feedbacks) / len(feedbacks), 2
        )
        product.update(rating=average, review_count=len(feedbacks))

    @classmethod
    def __flush_after_insert_event__(cls, target):
        super().__flush_after_insert_event__(target)
        cls._refresh_product_stats(target.product_id)
...
def get_product_list_context(query, obj):
    ...
    - brand, color, collar: attribute-based filtering
    """
    args_dict = {}
    ...
    # because product.price is a computed property (includes discounts)
    ...
    # Apply all attribute filters together
    if selected_filters:
    ...
    args_dict.update(attribute_filters)
